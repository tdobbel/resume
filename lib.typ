...
#let rust-icon = box(fa-icon("rust", fill: color-darknight))
#let python-icon = box(fa-icon("python", fill: color-darknight))
#let html-icon = box(fa-icon("html5", fill: color-darknight))
#let git-icon = box(fa-icon("git-alt", fill: color-darknight))
#let linux-icon = box(fa-icon("linux", fill: color-darknight))
...
#let resume(
  ...
  show-footer: true,
  language: "en",
  font: ("STIX Two Text", "Optima LT Pro", "Libertinus Serif",),
  header-font: ("Roboto"),
  paper-size: "a4",
  body,
) = {
  ...
  show heading.where(level: 1): it => [
    ...
    #text[#strong[#text(color, 13.5pt)[#it.body]]]
  ...
  pad[
    #grid(
      ...
      align(left)[
        ...
        #if type(items) == "array" [ #items.join(", ")
        ] else [
          #items
        ]
      ],
...
#let coverletter(
  ...
  set page(
    paper: paper-size,
    margin: (left: 25mm, right: 25mm, top: 15mm, bottom: 15mm),
